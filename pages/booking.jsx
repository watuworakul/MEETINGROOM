// pages/booking.jsx - multi-step booking form with email preview
const BookingPage = ({ prefill, onDone }) => {
    ...
    return (
      <div className="card" style={{ maxWidth: 520, margin: "60px auto", textAlign: "center", padding: "40px 32px" }}>
        ...
        <div className="muted" style={{ fontSize: 13, marginBottom: 22 }}>
          {lang === "th"
            ? `คำขอ #BK-${Math.floor(Math.random() * 9000 + 1000)} ถูกส่งให้เจ้าหน้าที่แล้ว`
            : `Request #BK-${Math.floor(Math.random() * 9000 + 1000)} sent to approver`}
        </div>
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", gap: 18, alignItems: "flex-start" }}>
      <div className="card">
          ...
          <div style={{ display: "flex", gap: 6, marginLeft: "auto" }}>
            {[1, 2, 3].map(n => (
              <div key={n} style={{
                ...
                background: step >= n ? "var(--accent)" : "var(--bg-2)",
                color: step >= n ? "var(--accent-ink)" : "var(--text-3)",
          ...
          {step === 1 && (
            <div className="col" style={{ gap: 14 }}>
                ...
                <div className="chip-row">
                  {ROOMS.map(r => (
                    <button key={r.id} className={`chip ${form.roomId === r.id ? "on" : ""}`} onClick={() => upd("roomId", r.id)}>
                      <RoomDot roomId={r.id} />
                      {r.short} · {r.capacity}{lang === "th" ? " คน" : "p"}
                    </button>
                  ))}
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: 10 }}>
                <div className="field">
                  <label className="field-label">{t("book_date")}</label>
                  <input className="input" type="date" value={form.date} onChange={e => upd("date", e.target.value)} />
                </div>
                <div className="field">
                  <label className="field-label">{t("book_time_start")}</label>
                  <input className="input mono" type="time" value={form.timeStart} onChange={e => upd("timeStart", e.target.value)} />
                </div>
                <div className="field">
                  <label className="field-label">{t("book_time_end")}</label>
                  <input className="input mono" type="time" value={form.timeEnd} onChange={e => upd("timeEnd", e.target.value)} />
                </div>
              </div>
              <div className="field">
            ...
            <div className="col" style={{ gap: 14 }}>
              ...
              <div className="field">
                <label className="field-label" style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  {ico.meet()} {lang === "th" ? "การเชื่อมต่อกับ Google" : "Google integration"}
                </label>
              </div>
              <label style={{ display: "flex", gap: 12, alignItems: "flex-start", cursor: "pointer", padding: "10px 12px", background: "var(--bg-2)", border: "1px solid var(--border)", borderRadius: 6 }}>
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: 500 }}>{t("book_gmeet")}</div>
                  <div className="field-hint">{t("book_gmeet_hint")}</div>
                </div>
              </label>
              <label style={{ display: "flex", gap: 12, alignItems: "flex-start", cursor: "pointer", padding: "10px 12px", background: "var(--bg-2)", border: "1px solid var(--border)", borderRadius: 6 }}>
                <input type="checkbox" checked={form.addToCalendar} onChange={e => upd("addToCalendar", e.target.checked)} style={{ marginTop: 2 }} />
                <div>
                  <div style={{ fontSize: 13, fontWeight: 500 }}>{t("book_calendar")}</div>
                  <div className="field-hint">{t("book_calendar_hint")}</div>
                </div>
              </label>
            </div>
          )}
            ...
            <div className="col" style={{ gap: 14 }}>
              ...
              <div>
                ...
                <div className="muted" style={{ fontSize: 11.5, marginTop: 8 }}>
                  {lang === "th"
                    ? "อีเมลนี้จะถูกส่งหลังจากเจ้าหน้าที่อนุมัติคำขอ"
                    : "This email is sent once the approver confirms the request."}
                </div>
              </div>
            </div>
          )}
        ...
        <div style={{ display: "flex", gap: 8, padding: "14px 16px", borderTop: "1px solid var(--border)", background: "var(--bg-2)" }}>
          ...
          {step < 3 ? (
            ...
            <button className="btn btn-primary" onClick={() => setSuccess(true)}>{ico.check()} {t("book_submit")}</button>
          )}
        </div>
      </div>
      ...
      <div className="col" style={{ gap: 12 }}>
        <div className="card">
          ...
          <div className="card-body" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            <div>
              ...
              <div className="chip-row">
                {room.amenities.map(a => <span key={a} className="chip" style={{ fontSize: 11.5 }}>{a}</span>)}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="card">
          <div className="card-head"><div className="card-title">{lang === "th" ? "การจองห้องนี้วันนี้" : "Bookings today"}</div></div>
          <div style={{ padding: "4px 0" }}>
            {BOOKINGS.filter(b => b.roomId === room.id && sameDay(b.start, new Date())).length === 0 ? (
              <div style={{ padding: "12px 14px", fontSize: 12, color: "var(--text-3)" }}>{lang === "th" ? "ยังว่างทั้งวัน" : "Free all day"}</div>
            ) : BOOKINGS.filter(b => b.roomId === room.id && sameDay(b.start, new Date())).map((b, i) => (
              <div key={b.id} style={{ display: "grid", gridTemplateColumns: "78px 1fr", padding: "8px 14px", borderTop: i === 0 ? 0 : "1px solid var(--border)", fontSize: 12, gap: 8 }}>
                <span className="mono muted">{fmtTime(b.start)}–{fmtTime(b.end)}</span>
                <span>{b.topic}</span>
              </div>
            ))}
          </div>
