...
const DashboardPage = ({ role, navigate }) => {
  ...
  const { rooms, bookings, eqRequests } = useData();

  const todays  = bookings.filter(b => sameDay(b.start, new Date())).sort((a, b) => a.start - b.start);
  const pending = bookings.filter(b => b.status === "pending").length + eqRequests.filter(r => r.status === "pending").length;
  const eqOut   = eqRequests.filter(r => r.status === "approved").reduce((s, r) => s + r.qty, 0);
        ...
        <div className="card">
          ...
          <div style={{ padding: "4px 0" }}>
            {todays.length === 0 ? (
              ...
              return (
                <div key={b.id} style={{ display: "grid", gridTemplateColumns: "70px 6px 1fr auto", gap: 12, alignItems: "center", padding: "11px 16px", borderTop: i === 0 ? 0 : "1px solid var(--border)" }}>
                  ...
                  <div style={{ width: 4, height: 36, borderRadius: 2, background: room?.color || "#888" }} />
                  <div>
        ...
        <div className="col">
          <div className="card">
            ...
            <div className="card-body" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <button className="btn btn-primary btn-lg" onClick={() => navigate("book")} style={{ justifyContent: "flex-start" }}>{ico.plus()} {t("qa_book")}</button>
              <button className="btn btn-lg" onClick={() => navigate("equipment")} style={{ justifyContent: "flex-start" }}>{ico.box()} {t("qa_borrow")}</button>
              <button className="btn btn-lg" onClick={() => navigate("calendar")} style={{ justifyContent: "flex-start" }}>{ico.calendar()} {t("qa_view_cal")}</button>
              <button className="btn btn-lg" onClick={() => navigate("history")} style={{ justifyContent: "flex-start" }}>{ico.history()} {t("qa_history")}</button>
            </div>
          </div>
            ...
            <div className="card-body" style={{ display: "flex", gap: 18, alignItems: "center" }}>
              ...
              <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
                {rooms.slice(0, 4).map((r, i) => {
                  const pct = [78, 64, 52, 41][i] || 50;
                  return (
                    <div key={r.id}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11.5, marginBottom: 4 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}><RoomDot roomId={r.id} /><span>{r.short}</span></div>
