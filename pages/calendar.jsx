...
const CalendarPage = ({ navigate, openBooking, selectedBookingId, setSelectedBookingId }) => {
  ...
  const eventStyle = (b) => {
    ...
    return { top: (startMin / 60) * cellH, height: (dur / 60) * cellH - 2 };
  };
        ...
        <div className={`cal-grid ${view === "day" ? "day" : ""}`}>
          ...
          {days.map((d, i) => (
            <div key={i} className={`cal-day-head ${sameDay(d, new Date()) ? "today" : ""}`}>
              <div className="cal-day-name">{new Intl.DateTimeFormat(lang === "th" ? "th-TH-u-ca-gregory" : "en-GB", { weekday: "short" }).format(d)}</div>
          ...
          {days.map((d, di) => (
            <div key={di} className="cal-col">
              {hours.map((h, i) => (
                <div key={i} className="cal-cell" style={{ height: cellH }}
                  onClick={() => openBooking({ date: d, hour: h, roomId: roomFilter !== "all" ? roomFilter : undefined })} />
              ))}
              {showNow && sameDay(d, now) && <div className="now-line" style={{ top: nowPx }} />}
              {visible.filter(b => sameDay(b.start, d)).map(b => {
                ...
                return (
                  <div key={b.id} className={`cal-event ${cls}`}
                    style={{ ...eventStyle(b), borderLeftColor: room?.color || "#888" }}
    ...
    <Modal open={!!b} onClose={onClose} title={b.topic} sub={`${room?.name} · ${fmtDate(b.start, lang)}`} width={520}
      ...
      <div className="col" style={{ gap: 12 }}>
        <div style={{ display: "grid", gridTemplateColumns: "100px 1fr", gap: 10, fontSize: 13 }}>
          <span className="muted">{lang === "th" ? "สถานะ" : "Status"}</span><span><Badge status={b.status} /></span>
          ...
          <span className="muted">{lang === "th" ? "ผู้จอง" : "Organizer"}</span><span>{b.requester} <span className="muted">&lt;{b.email}&gt;</span></span>
